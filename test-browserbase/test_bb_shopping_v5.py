...
############################
# 2) Cookie management functions (reused from local testing file)
############################
...
def store_cookies(page: Page):
    ...
    all_cookies = page.context.cookies(SITE_URL)
...
def check_login_status(page: Page) -> bool:
    ...
    page.goto(f"{SITE_URL}/customer/account/")
...
def authenticate(page: Page):
    ...
    form_elements = page.query_selector_all("form.form-login input, form#login-form input")
    ...
    # Check for Magento session cookies
...
def run_flow(page: Page):
    ...
    2) Try to restore cookies first
    3) Check login status, if not logged in, perform full authentication
    ...
    cookies_restored = restore_cookies(page)
    
    if cookies_restored and check_login_status(page):
...
def main():
    with sync_playwright() as p:
        ...
        # Import webbrowser to open the URL automatically
        import webbrowser
        print("Opening debugger URL in your default browser...")
        webbrowser.open(debug_info.debugger_url)
        page.pause()
