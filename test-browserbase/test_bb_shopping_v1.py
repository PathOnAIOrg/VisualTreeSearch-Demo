...
def set_required_magento_cookies(browser_tab: Page):
    """Set required Magento cookies that might be missing after login attempts."""
    print("\nChecking for required Magento cookies...")
    
    cookies = browser_tab.context.cookies()
    cookie_names = {c["name"] for c in cookies}
    
    # List of modifications we need to make
    cookies_to_add = []
    
    # Check for X-Magento-Vary cookie
    if "X-Magento-Vary" not in cookie_names:
        print("⚠️ Missing X-Magento-Vary cookie - adding it manually")
        # Generate a random value similar to the one seen in local browser
        vary_value = uuid.uuid4().hex[:32]  # 32 char hex string
        cookies_to_add.append({
            ...
            "httpOnly": True,
            "sameSite": "Lax",
            # Set expiry 1 year in the future (similar to what browser shows)
            "expires": time.time() + 365 * 24 * 60 * 60
        })
    
    # Check for private_content_version cookie
    if "private_content_version" not in cookie_names:
        print("⚠️ Missing private_content_version cookie - adding it manually")
        # Generate a random value similar to the one seen in local browser
        content_version = uuid.uuid4().hex[:32]  # 32 char hex string
        cookies_to_add.append({
    ...
    # Add the cookies if needed
    if cookies_to_add:
        browser_tab.context.add_cookies(cookies_to_add)
        print(f"Added {len(cookies_to_add)} missing Magento cookies")
        
        # Print the updated cookies
        print("\n### Updated Cookies (Markdown Table)\n")
        print_cookie_table_markdown(browser_tab.context.cookies())
        print()
    else:
        print("✅ All required Magento cookies are present")
...
def authenticate(browser_tab: Page):
    ...
    # Check if login succeeded (partial check)
    ...
    if magento_session_cookies:
        ...
        # Set required cookies that might be missing
        set_required_magento_cookies(browser_tab)
    else:
        print("❌ No Magento 'frontend' or 'PHPSESSID' cookie found - likely not authenticated.\n")
    
    # Save the cookies after adding the required ones
    store_cookies(browser_tab)
    
    # Navigate to account page to confirm login
    print("\nNavigating to account page to verify login...\n")
    ...
    if is_logged_in:
        ...
        # Try alternative approach - add cookies again after page load
        print("Trying alternative approach - adding required cookies after page load...\n")
        set_required_magento_cookies(browser_tab)
        ...
        # Check login again
        if "Login" not in browser_tab.title():
            print(f"✅ Alternative approach worked! Now on page: {browser_tab.title()}")
            return True
        else:
            # Print partial page content for debugging
...
def run(browser_tab: Page):
    ...
    if "Login" in browser_tab.title():
        ...
        # One more attempt - try setting required cookies and refreshing
        print("\nLast attempt - adding required cookies and refreshing...\n")
        set_required_magento_cookies(browser_tab)
        browser_tab.reload()
        browser_tab.wait_for_load_state("networkidle")
        
        if "Login" not in browser_tab.title():
            print("✅ Last attempt worked! Successfully accessing protected page.")
            browser_tab.screenshot(path="screenshot_final_success.png")
        else:
            print("❌ All attempts failed. Still on login page.")
    else:
