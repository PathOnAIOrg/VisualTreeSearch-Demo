...
# Load environment variables if needed (not strictly necessary if you're just running locally)
load_dotenv()
...
def authenticate(browser_tab: Page):
    ...
    login_button = (
        browser_tab.query_selector(".action.login.primary")
        or browser_tab.query_selector("#send2")
        or browser_tab.query_selector("button[type='submit']")
    )
